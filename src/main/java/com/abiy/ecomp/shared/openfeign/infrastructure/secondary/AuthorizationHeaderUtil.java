package com.abiy.ecomp.shared.openfeign.infrastructure.secondary;

import com.abiy.ecomp.shared.error.domain.Assert;
import com.abiy.ecomp.shared.openfeign.infrastructure.model.OAuthIdpTokenResponse;
...
public class AuthorizationHeaderUtil {
    ...
    private OAuth2AccessTokenResponse refreshTokenClient(OAuth2AuthorizedClient currentClient) {
        ...
        var requestEntity =
            RequestEntity.post(URI.create(currentClient.getClientRegistration().getProviderDetails().getTokenUri())).contentType(MediaType.APPLICATION_FORM_URLENCODED).body(formParameters);
        Assert.notNull("Request Entity", requestEntity);
        try {
            ...
            ResponseEntity<OAuthIdpTokenResponse> responseEntity = r.exchange(requestEntity, OAuthIdpTokenResponse.class);
    ...
    private OAuth2AccessTokenResponse toOAuth2AccessTokenResponse(OAuthIdpTokenResponse oAuthIdpResponse) {
        Map<String, Object> additionalParameters = new HashMap<>();
        additionalParameters.put("id_token", oAuthIdpResponse.idToken());
        additionalParameters.put("not-before-policy", oAuthIdpResponse.notBefore());
        additionalParameters.put("refresh_expires_in", oAuthIdpResponse.refreshExpiresIn());
        additionalParameters.put("session_state", oAuthIdpResponse.sessionState());
        return OAuth2AccessTokenResponse.withToken(oAuthIdpResponse.accessToken()).expiresIn(oAuthIdpResponse.expiresIn()).refreshToken(oAuthIdpResponse.refreshToken()).scopes(Pattern.compile("\\s").splitAsStream(oAuthIdpResponse.scope()).collect(Collectors.toSet())).tokenType(OAuth2AccessToken.TokenType.BEARER).additionalParameters(additionalParameters).build();
    }

    private RestTemplate restTemplate(String clientId, String clientSecret) {
        return restTemplateBuilder.additionalMessageConverters(new FormHttpMessageConverter(), new OAuth2AccessTokenResponseHttpMessageConverter()).errorHandler(new OAuth2ErrorResponseErrorHandler()).basicAuthentication(clientId, clientSecret).build();
    }
