...
```java
public EcompApplicationPage<Beer> list(EcompApplicationPageable pagination) {
  // ...
}
...
ResponseEntity<RestEcompApplicationPage<RestBeer>> list(@Validated RestEcompApplicationPageable pagination) {
  return ResponseEntity.ok(RestEcompApplicationPage.from(beers.list(pagination.toPageable()), RestBeer::from));
}
